 ...
 *     #include "main.h"
 *
 *     RETRAINER_TEST(returns_the_sum, "c.basics.functions") {
 *         RETRAINER_ASSERT_INT(add(2, 3), 5);
 *     }
 *
 * Note what it does *not* end with. The entry point lives in
 * `retrainer_main.c`, which the runtime compiles alongside every test file.
...
void retrainer_register(const char *name, const char *concept, const char *file, int line,
                        void (*body)(void));
...
int retrainer_run(const char *report_path) {
    ...
    for (int i = 0; i < retrainer_count; i++) {
        ...
        if (entry->concept && entry->concept[0]) {
            fputs(",\"concept\":", out);
            retrainer_json(out, entry->concept);
        }
        if (retrainer_failed) {
            fputs(",\"message\":", out);
